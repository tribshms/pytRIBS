...
def get_mrf_water_balance(instance, method):
    """
    """
    waterbalance = _run_mrf_water_balance(instance,method)
    instance.mrf['waterbalance'] = waterbalance


def get_element_water_balance(instance, method):
    """
    ...
    Parameters:
    - instance: An instance of the model containing element results.
    - method: A string specifying the calculation method ('full') or a list of custom date ranges (min and max).

    Example:
    1. Calculate full water balance: get_element_water_balance(instance, 'full')
    2. Calculate custom water balance for specific dates: get_element_water_balance(instance, ['2024-01-01', '2024-01-31'])
    """
    ...
    for n in nodes:
        ...
        if isinstance(instance.element[n], dict):
            data = instance.element[n]['pixel']  # waterbalance calcs have already been run
        else:
            data = instance.element[n]


        waterbalance = _run_element_water_balance(instance, n, method)
        instance.element[n]["waterbalance"] = waterbalance


# WATER BALANCE FUNCTIONS
def _run_element_water_balance(instance, element_id, method):
    """
    ...
    """
    data = instance.element[element_id]
    ...
    if method == 'full':
        ...
        waterbalance = _element_wb_components(instance, element_id, min_date, max_date)
        ...
        for n in range(0, len(timeframe)):
            if n == 0:
                waterbalance = _element_wb_components(instance, element_id, begin[n], end[n])
            else:
                temp = _element_wb_components(instance, element_id, begin[n], end[n])
...
@staticmethod
def _element_wb_components(instance, element_id, begin, end):
    """
    ...
    df = instance.get_element_wb_dataframe(element_id)
...
def _run_mrf_water_balance(instance, method):
    """
    ...
    """
    global waterbalance
    data = instance.mrf['mrf']

    if method == 'full':
        min_date = min(data.Time)
        max_date = max(data.Time)
        waterbalance = _mrf_wb_components(instance, min_date, max_date)
        timeframe = data.Time.mean()
        ...
        for n in range(0, len(timeframe)):
            if n == 0:
                waterbalance = _mrf_wb_components(instance, begin[n], end[n])
            else:
                temp = _mrf_wb_components(instance, begin[n], end[n])
...
@staticmethod
def _mrf_wb_components(instance, begin, end):
    """
    ...
    df = instance.get_mrf_wb_dataframe()
    duration_id = (df['Time'].values >= begin) & (df['Time'].values <= end)
    df = df[duration_id]
